      ...
      <el-card
        ...
        <template v-if='submitDetail!==undefined&&submitDetail!==null' #default>
          ...
          >
            ...
            <el-descriptions-item v-if='submitDetail.stderr?.length??0>0' label='错误信息' label-class-name='content-label'>
              {{ submitDetail.stderr }}
            </el-descriptions-item>
          </el-descriptions>
        </template>
      ...
      <div class='toolbar'>
        <el-row :gutter='20'>
          ...
          <el-col :span='10'>
            <el-button-group>
              <el-button
                type='primary' size='large' :loading='fetchingSubmitList' :icon='ArrowLeft' :disabled='!havePrev'
            ...
            <template v-if='submitList'>
              <el-table
                ...
                <el-table-column prop='castMemory' label='消耗内存(KB)'>
                  <template #default='scope'>
                    <span>{{ /\d+/.test(scope.row.castMemory) ? scope.row.castMemory / 1024 : '--' }}</span>
                  </template>
                </el-table-column>
