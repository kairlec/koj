...
(
    ...
    cast_memory           BIGINT                                 NULL COMMENT '任务内存',
    cast_time             BIGINT                                 NULL COMMENT '耗时',
...
CREATE TABLE IF NOT EXISTS koj.submit_extend
(
    id          BIGINT                                 NOT NULL COMMENT '任务id',
    `code`      TEXT                                   NOT NULL COMMENT '代码',
    stdout      TEXT                                   NOT NULL COMMENT '标准输出',
    stderr      TEXT                                   NOT NULL COMMENT '标准错误',
    create_time DATETIME DEFAULT NOW()                 NOT NULL COMMENT '创建时间',
    update_time DATETIME DEFAULT NOW() ON UPDATE NOW() NOT NULL COMMENT '更新时间',
    PRIMARY KEY (id),
    FOREIGN KEY (id) REFERENCES koj.submit (id) ON DELETE CASCADE ON UPDATE CASCADE
) COMMENT ='任务拓展信息表';
...
#     spj_content TEXT                                   NULL COMMENT 'spj内容',
    ...
    max_output_size    BIGINT UNSIGNED   DEFAULT (~0)                  NOT NULL COMMENT '最大输出限制(字节),默认无限制',
    max_stack          BIGINT UNSIGNED   DEFAULT (~0)                  NOT NULL COMMENT '最大堆栈(字节),默认无限制',
    max_process_number SMALLINT UNSIGNED DEFAULT 1                     NOT NULL COMMENT '最大处理器数量,默认1',
    create_time        DATETIME          DEFAULT NOW()                 NOT NULL COMMENT '创建时间',
    update_time        DATETIME          DEFAULT NOW() ON UPDATE NOW() NOT NULL COMMENT '更新时间',
    args               TEXT                                            NOT NULL COMMENT '额外参数(默认无)',
    env                TEXT                                            NOT NULL COMMENT '额外环境参数(默认无)',
    PRIMARY KEY (problem_id, language_id),
    INDEX problem_language_idx (problem_id, language_id),
